    ...
    <div className="w-full py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {organizations.map((org, idx) => (
          <div key={idx} className={`p-5 rounded-xl border ${org.color} bg-opacity-40 shadow-sm hover:shadow-md transition-all`}>
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-lg bg-white/50 backdrop-blur-sm`}>{org.icon}</div>
                <div>
                  <h3 className="font-bold text-lg leading-tight">{org.name}</h3>
                  <span className="text-xs font-semibold uppercase tracking-wider opacity-75">{org.type}</span>
                </div>
              </div>
            </div>
            
            <p className="text-sm opacity-90 mb-4 leading-relaxed">{org.desc}</p>

            <div className="flex flex-col gap-2">
              {org.isEmergency ? (
                <div className="grid grid-cols-2 gap-2 mt-1">
                  {org.contacts.map(c => (
                    <a key={c.number} href={`tel:${c.number}`} className="flex items-center justify-center gap-2 bg-red-600 text-white py-2 rounded-lg text-sm font-bold hover:bg-red-700 transition">
                ...
                <div className="flex flex-wrap gap-3 text-sm font-medium">
                  {org.phone && <a href={`tel:${org.phone}`} className="flex items-center gap-1 hover:underline"><Phone className="h-3 w-3" /> {org.phone}</a>}
                  {org.email && <a href={`mailto:${org.email}`} className="flex items-center gap-1 hover:underline"><Mail className="h-3 w-3" /> {t('communitySupport.email')}</a>}
                  {org.website && <a href={org.website} target="_blank" rel="noreferrer" className="flex items-center gap-1 hover:underline"><Globe className="h-3 w-3" /> {t('communitySupport.website')}</a>}
                </div>
              )}
            </div>
          </div>
